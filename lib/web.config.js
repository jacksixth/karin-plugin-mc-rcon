...
export default {
    ...
    /** 动态渲染的组件 */
    components: () => [
        components.accordionPro.create("servers", config().servers.map((item, index) => {
            return {
                ...item,
                title: item.alias,
        ...
        components.accordionPro.create("QQNoLinkMcNickname", config().QQNoLinkMcNickname.map((item, index) => {
            return {
                ...item,
