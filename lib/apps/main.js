import { config, dirConfig, isValidHost, isValidPort, mcMotd, rconClient, } from "../utils/index.js";
import karin, { segment, writeJsonSync } from "node-karin";
//是否正在修改文件
let isWriteJSON = false;
//motd查询 #motd <host>:<port> | #motd <host> <port>
export const motd = karin.command(/^#motd\s+([a-z0-9.-]+)(?::(\d+)|\s+(\d+))$/i, async (e) => {
    ...
    if (!regRes) {
        ...
        if (data) {
            ...
            msgList.push(segment.text(data.players.online + "/" + data.players.max + "\n"));
            if (data.description)
...
export const addServer = karin.command(/^#(?:server add|添加服务器) (\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)$/i, async (e) => {
    if (isWriteJSON) {
    ...
    const regRes = e.msg.match(/^#(?:server add|添加服务器) (\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)$/i);
    if (!regRes) {
        //不可能会走到这里
        return;
    }
        ...
        if (_config.servers.find((item) => item.alias === alias)) {
            e.reply("❌ 该服务器别名已存在别名不可重复，请修改后重试,当前存在服务器：" +
                _config.servers.map((item) => item.alias).join(","), { reply: true });
            return;
        }
...
export const removeServer = karin.command(/^#(?:server remove|删除服务器) (\S+)$/i, async (e) => {
    if (isWriteJSON) {
        e.reply("❌ 其他命令正在修改文件，请稍后再试", { reply: true });
        return;
    }
...
let isListing = false;
export const listServer = karin.command(/^#(?:server list|服务器状态)$/i, async (e) => {
    const _config = config();
    if (_config.banQQ.includes(e.sender.userId)) {
        e.reply("❌ 你被封禁了", { reply: true });
    }
    ...
    for (let index = 0; index < _config.servers.length; index++) {
        ...
        msgList.push(segment.text(`${element.alias} ${element.host}${element.port == "25565" ? ":" + element.port : ""}\n`));
        try {
            ...
            if (motd) {
                ...
                msgList.push(segment.text(`服务器版本：${motd.type} ${motd.version}\n服务器人数:${motd.players.online}/${motd.players.max}\n`));
                const rcon = new rconClient(element.host, element.rconPort, element.password);
                try {
                    ...
                    if (listRes) {
                        ...
                        if (listRes.includes(":") && listRes.split(":").length > 1)
                            msgList.push(segment.text("在线玩家：" + listRes.split(":")[1] + "\n"));
        ...
        nodeMsg.push(segment.node(e.bot.account.selfId, e.bot.account.name, msgList));
    }
...
export const serverInfo = karin.command(/^#(?:server info|服务器信息) (\S+)$/i, async (e) => {
    const _config = config();
    if (_config.banQQ.includes(e.sender.userId)) {
        e.reply("❌ 你被封禁了", { reply: true });
    }
    ...
    let msgList = [];
    msgList.push(segment.text(`${findServer.alias} ${findServer.host}${findServer.port == "25565" ? ":" + findServer.port : ""}\n`));
    try {
        ...
        if (motd) {
            ...
            msgList.push(segment.text(`服务器版本：${motd.type} ${motd.version}\n服务器人数:${motd.players.online}/${motd.players.max}\n`));
            const rcon = new rconClient(findServer.host, findServer.rconPort, findServer.password);
            try {
                const listRes = await rcon.send("list");
                if (listRes) {
                    //返回的list信息处理 中文冒号和英文冒号都可能存在
...
export const wlon = karin.command(/^#(?:server wlon|开启白名单) (\S+)$/i, async (e) => {
    const _config = config();
    if (!e.isAdmin && !e.isMaster) {
        e.reply("❌ 你没有权限使用该命令", { reply: true });
        return;
...
export const wloff = karin.command(/^#(?:server wloff|关闭白名单) (\S+)$/i, async (e) => {
    const _config = config();
    if (!e.isAdmin && !e.isMaster) {
        e.reply("❌ 你没有权限使用该命令", { reply: true });
        return;
...
export const wllist = karin.command(/^#(?:server wllist|白名单列表) (\S+)$/i, async (e) => {
    const _config = config();
    if (!e.isAdmin && !e.isMaster) {
        e.reply("❌ 你没有权限使用该命令", { reply: true });
        return;
...
let isSending = false;
export const rcon = karin.command(/^#(?:rcon|发送命令) (\S+) (.+)$/i, async (e) => {
    if (!e.isAdmin && !e.isMaster) {
        e.reply("❌ 你没有权限使用该命令", { reply: true });
        return;
    }
...
export const bind = karin.command(/^#(?:bind|绑定|注册) (\S+)$/i, async (e) => {
    const _config = config();
    if (_config.banQQ.includes(e.sender.userId)) {
        e.reply("❌ 你被封禁了", { reply: true });
    }
    ...
    if (!regRes) {
        ...
        const findQQ = _config.QQNoLinkMcNickname.find((item) => item.qqNo === e.sender.userId);
        if (findQQ) {
        ...
        const findNickname = _config.QQNoLinkMcNickname.find((item) => item.mcNickname === nickName);
        if (findNickname) {
            try {
                ...
                e.reply(`❌ 该昵称已被绑定且主人已不在群聊中，请联系管理员使用 #unbind <${findNickname.qqNo}> 解绑`, {
                    reply: true,
                });
            }
            return;
...
//解绑游戏昵称 #unbind
export const unbind = karin.command(/^#(?:unbind|解绑|注销)$/i, async (e) => {
    const _config = config();
    if (_config.banQQ.includes(e.sender.userId)) {
        e.reply("❌ 你被封禁了", { reply: true });
    }
    ...
    if (!regRes) {
        ...
        _config.QQNoLinkMcNickname = _config.QQNoLinkMcNickname.filter((item) => item.qqNo !== e.sender.userId);
        isWriteJSON = true;
        writeJsonSync(`${dirConfig}/config.json`, _config);
...
export const unbindAdmin = karin.command(/^#(?:unbind|解绑|注销) (\d+)$/i, async (e) => {
    if (!e.isAdmin && !e.isMaster) {
        e.reply("❌ 你没有权限使用该命令", { reply: true });
        return;
    }
    ...
    if (!regRes) {
        ...
        _config.QQNoLinkMcNickname = _config.QQNoLinkMcNickname.filter((item) => item.qqNo !== qqNo);
        isWriteJSON = true;
        writeJsonSync(`${dirConfig}/config.json`, _config);
        e.reply(`✅ 解绑成功`, { reply: true });
        isWriteJSON = false;
...
export const ban = karin.command(/^#(?:ban|封禁) (\d+)$/i, async (e) => {
    if (!e.isAdmin && !e.isMaster) {
        e.reply("❌ 你没有权限使用该命令", { reply: true });
        return;
    }
    ...
    if (findQQ) {
        for (let index = 0; index < _config.servers.length; index++) {
            ...
            try {
                ...
                msgList.push(segment.text(`✅ ${element.alias}服务器已ban${findQQ.mcNickname}\n命令返回：${res}\n`));
            }
            catch (error) {
                //忽略错误
                msgList.push(segment.text(`❌ ${element.alias}服务器ban命令执行失败:${error}\n`));
            }
        }
    }
    else {
...
export const unban = karin.command(/^#(?:unban|解封) (\d+)$/i, async (e) => {
    if (!e.isAdmin && !e.isMaster) {
        e.reply("❌ 你没有权限使用该命令", { reply: true });
        return;
    }
    ...
    if (findQQ) {
        for (let index = 0; index < _config.servers.length; index++) {
            ...
            try {
                ...
                msgList.push(segment.text(`✅ ${element.alias}服务器已解ban${findQQ.mcNickname}\n命令返回：${res}\n`));
            }
            catch (error) {
                msgList.push(segment.text(`❌ ${element.alias}服务器解ban命令执行失败:${error}\n`));
            }
        }
    }
    else {
...
export const apply = karin.command(/^#(?:apply|申请) (\S+)$/i, async (e) => {
    const _config = config();
    if (_config.banQQ.includes(e.sender.userId)) {
        e.reply("❌ 你被封禁了", { reply: true });
    }
...
export const revoke = karin.command(/^#(?:revoke|取消白名单)\s+(\d+)\s+(\S+)$/, async (e) => {
...
// 群成员退群通知事件
export const groupMemberBanNotice = karin.accept("notice.groupMemberBan", async (e) => {
    //退群 清除绑定 在各服务器中ban掉
    const _config = config();
    if (isWriteJSON) {
    ...
    const findNickname = _config.QQNoLinkMcNickname.find((item) => item.qqNo == e.userId);
    if (!findNickname) {
        msgList.push(segment.text("该QQ号未绑定昵称\n"));
        return;
    }
    for (let index = 0; index < _config.servers.length; index++) {
        ...
        try {
            ...
            msgList.push(segment.text(`✅ ${element.alias}服务器已ban${findNickname.mcNickname}\n命令返回：${res}\n`));
        }
        catch (error) {
            msgList.push(segment.text(`❌ ${element.alias}服务器ban命令执行失败:${error}\n`));
        }
...
//入群欢迎
export const groupMemberAddNotice = karin.accept("notice.groupMemberAdd", async (e) => {
    //加群
    e.bot.sendMsg(e.contact, [
        segment.at(e.userId),
    ...
    //查看是否有绑定
    const _config = config();
    const findQQ = _config.QQNoLinkMcNickname.find((item) => item.qqNo === e.userId);
    if (findQQ) {
        e.bot.sendMsg(e.contact, [
            segment.at(e.userId),
            segment.text(`你已经绑定了昵称：${findQQ.mcNickname}，如无法加入服务器请联系管理员解除服务器黑名单`),
        ]);
    }
}, {
...
//命令列表
// 查看命令帮助 #voidhelp
export const voidhelp = karin.command(/^#(?:voidhelp|void帮助)$/i, async (e) => {
    const helpList = [
        {
            cmd: "#motd <host>:<port>",
